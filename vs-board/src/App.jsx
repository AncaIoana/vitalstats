...
const INITIAL_STORIES = [
  ...
  { id: "s15", phase: "phase1", epic: "dbt", title: "Write stg_blood_tests.sql", status: "backlog", priority: "high", notes: "Clean columns, cast types, parse dates, generate analyte_slug, unit normalisation. Output: silver.stg_google_sheets__blood_tests_vw" },
  { id: "s16", phase: "phase1", epic: "dbt", title: "Write stg_menoscale.sql", status: "backlog", priority: "medium", notes: "Clean date, validate score 0-100. Output: silver.stg_google_sheets__menoscale_vw" },
  ...
  // Analysis — once data is flowing through Gold
  ...
  // Phase 1b — medications & vaccines (after core blood tests pipeline is stable)
  { id: "s63", phase: "phase1", epic: "Ingestion", title: "Write extract_medications.py for Medications tab", status: "backlog", priority: "high", notes: "Read Medications tab from MEDICATIONS_SHEET_ID. Parse dates (handle bad date edge cases). Normalise medication slugs. Hash dedup. See medications-vaccines-schema.md. Loads into raw.medications_raw (not silver directly)." },
  { id: "s64", phase: "phase1", epic: "Ingestion", title: "Write extract_vaccines.py for Vaccines tab", status: "backlog", priority: "high", notes: "Read Vaccines tab from MEDICATIONS_SHEET_ID. Filter WHERE Name = TRACKED_PERSON_NAME (env var) — discard partner rows. Strip trailing spaces from vaccine names. Parse booster_due float year. Parse dose_number from vaccine type string (regex). Parse is_most_recent (Yes/No → boolean). Parse expiration_date (month-year → YYYY-MM-01). New columns: administration_site, vaccination_location, batch_lot_number, vaccine_product_name, manufacturer — all nullable free text. Add EXPECTED_COLUMNS_VACCINES + KNOWN_VACCINES to known_values.py. Source column is Date Administered. See medications-vaccines-schema.md. Loads into raw.vaccines_raw (not silver directly)." },
  { id: "s65", phase: "phase1", epic: "Database", title: "Add raw.medications_raw + raw.vaccines_raw to create_ingestion_tables.sql", status: "backlog", priority: "high", notes: "Append-only raw tables for medications and vaccines. Follow same pattern as raw.blood_tests_raw. Add to create_ingestion_tables.sql. dbt will create silver views — do not pre-create silver tables." },
  { id: "s66", phase: "phase1", epic: "dbt", title: "Write stg_medications.sql", status: "backlog", priority: "high", notes: "Clean names, parse dosage, compute is_active. medication_slug generation. Output: silver.stg_google_sheets__medications_vw. Reads from: raw.medications_raw." },
  { id: "s67", phase: "phase1", epic: "dbt", title: "Write stg_vaccines.sql", status: "backlog", priority: "high", notes: "Clean vaccine names, parse dose_number from vaccine type string, parse booster_due, derive booster_due_date. Parse is_most_recent boolean. Parse expiration_date (month-year → DATE). Pass through nullable fields: administration_site, vaccination_location, batch_lot_number, vaccine_product_name, manufacturer. Filter to tracked person confirmed at this layer too. Output: silver.stg_google_sheets__vaccines_vw. Reads from: raw.vaccines_raw."},
  { id: "s68", phase: "phase1", epic: "dbt", title: "Add medications + vaccines to mart_health_timeline", status: "backlog", priority: "medium", notes: "event_type = 'medication' and 'vaccine'. Medications add a row per active period start. Vaccines add a row per administration." },
  { id: "s69", phase: "phase1", epic: "dbt", title: "Add medication active flags to mart_ml_features", status: "backlog", priority: "medium", notes: "med_<slug>_active binary features for each tracked medication — critical for blood test interpretation. See database-schema.md for full feature list." },
  { id: "s71", phase: "phase1", epic: "Ingestion", title: "Build parser validation notification layer", status: "backlog", priority: "medium", notes: "Non-blocking warnings for suspicious but parseable values: implausible dates, extreme outliers, fuzzy-matched names. Log to stg_unknown_values + pipeline_run_log. ADR-017. Implement after Phase 1 core pipeline is stable." },

  // ── PHASE 2 ────────────────────────────────────────────────────────────────
  // Billing alerts FIRST — before any AWS resource is created
  { id: "s55", phase: "phase2", epic: "Security", title: "Set AWS billing alerts ($5 and $20)", status: "backlog", priority: "high", notes: "Set before creating any AWS resources. AWS Billing → Budgets. Alerts notify but do not stop charges. ADR-016." },
  ...
  // Terraform — infrastructure as code before anything is manually created
  ...
  // Security controls — defined in Terraform alongside infrastructure
  { id: "s56", phase: "phase2", epic: "Security", title: "Implement IAM least privilege roles", status: "backlog", priority: "high", notes: "Separate IAM roles for ingestion, dbt, ML. Each role has only the permissions it needs. Define in Terraform. ADR-016." },
  ...
  { id: "s59", phase: "phase2", epic: "Security", title: "Enable S3 + RDS encryption at rest", status: "backlog", priority: "high", notes: "Free on AWS. One Terraform line each. S3: server-side encryption. RDS: storage_encrypted = true. ADR-016." },
  { id: "s60", phase: "phase2", epic: "Security", title: "Enable AWS CloudTrail", status: "backlog", priority: "medium", notes: "Logs every AWS API call. Free for management events. Invaluable for auditing. Enable on day one. ADR-016." },

  // Docker — containerise before moving to cloud
  ...
  // Move to cloud — once infrastructure is defined and Docker works
  ...
  // Scheduling — last, once everything runs cleanly in the cloud
  { id: "s35", phase: "phase2", epic: "CI/CD", title: "Scheduled pipeline via GitHub Actions", status: "backlog", priority: "high", notes: "pipeline.yml. Runs every Sunday night. Ingest then transform then email summary via SES." },

  // ── PHASE 3 ────────────────────────────────────────────────────────────────
  // Start simple — rule-based baseline before any ML
  ...
  // RAG enrichment before LLM — so context is available for prompts
  ...
  { id: "s39", phase: "phase3", epic: "AI", title: "LLM-powered plain English insights", status: "backlog", priority: "high", notes: "Feed flagged results to Claude API. Store prompts/responses in gold.mart_insights." },
  { id: "s62", phase: "phase3", epic: "Security", title: "Review health data in LLM prompts", status: "backlog", priority: "medium", notes: "Prefer aggregated values in prompts. Store all prompts + responses in gold.mart_insights. LLM API keys in Parameter Store. ADR-016." },
  ...
  // Cross-source ML — medications context enriches blood test models
  { id: "s70", phase: "phase3", epic: "ML", title: "Cross-source ML: blood tests + medications", status: "backlog", priority: "medium", notes: "Ferritin vs ferrous sulfate dose. ALT vs clinically significant medication active periods. Cholesterol vs Simvastatin. Medication active flag as feature in all blood test models.Ferritin vs ferrous sulfate dose. ALT vs clinically significant medication active periods. Cholesterol vs Simvastatin. Medication active flag as feature in all blood test models." },

  // Orchestration — last, once all ML jobs exist to orchestrate
  ...
  // ── PHASE 4+ ───────────────────────────────────────────────────────────────
  // Fitbit — API integration first, then staging models
  ...
  // Flo — export parsing, then ML integration
  { id: "s49", phase: "phase4", epic: "Flo", title: "Parse Flo GDPR JSON export", status: "backlog", priority: "medium", notes: "Manual export from Flo app (Settings → Privacy → Export data). Parse into stg_flo_cycles and stg_flo_symptoms." },
  { id: "s50", phase: "phase4", epic: "Flo", title: "Add cycle phase as ML feature across all blood test models", status: "backlog", priority: "medium", notes: "Join stg_flo_cycles to mart_ml_features on date. cycle_phase adds hormonal context to every blood test result." },
  ...
  // Frontend — dashboard before cross-source ML so you can see the results
  ...
  // Cross-source ML — needs all sources loaded first
  ...
  { id: "s51", phase: "phase4", epic: "Flo", title: "Cross-source ML: blood tests + Flo cycle data", status: "backlog", priority: "low", notes: "ALT vs cycle phase. Ferritin vs period heaviness. MenoScale score vs cycle regularity." },

  // DevOps housekeeping — low priority, evaluate when active coding is underway
  { id: "s52", phase: "phase4", epic: "DevOps", title: "Explore migrating board to GitHub Projects", status: "backlog", priority: "low", notes: "Once active coding begins, GitHub Projects integrates with issues, PRs, and commits. Evaluate when Phase 1 is underway." },
  { id: "s72", phase: "phase4", epic: "DevOps", title: "Host kanban board on GitHub Pages", status: "backlog", priority: "low", notes: "Wrap vitalstats-board-local.jsx in a Vite React project. Build with npm run build. Deploy to gh-pages branch. Set up GitHub Actions to auto-rebuild on push to main. Free for public repos. URL: your-username.github.io/vitalStats/board." },
];
...
export default function VitalStatsBoard() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("vskanban1");
      if (saved) {
        const parsed = JSON.parse(saved);
        const savedIds = new Set(parsed.map(s => s.id));
        const merged = [...parsed, ...INITIAL_STORIES.filter(s => !savedIds.has(s.id))];
        setStories(merged);
      }
    } catch (_) { }
    setLoaded(true);
  ...
  const save = useCallback((updated) => {
    setSaving(true);
    try {
      localStorage.setItem("vskanban1", JSON.stringify(updated));
    } catch (_) { }
