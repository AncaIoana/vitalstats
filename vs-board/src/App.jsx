...
const INITIAL_STORIES = [
  ...
  { id: "s64", phase: "phase1", epic: "Ingestion", title: "Write extract_vaccines.py for Vaccines tab", status: "backlog", priority: "high", notes: "Read Vaccines tab from MEDICATIONS_SHEET_ID. Filter WHERE Name = TRACKED_PERSON_NAME (env var) — discard partner rows. Strip trailing spaces from vaccine names. Parse booster_due float year. Parse is_most_recent (Yes/No → boolean). Parse expiration_date (month-year → YYYY-MM-01). New columns: administration_site, vaccination_location, batch_lot_number, vaccine_product_name, manufacturer — all nullable free text. Add EXPECTED_COLUMNS_VACCINES + KNOWN_VACCINES to known_values.py. Source column is Date Administered. See medications-vaccines-schema.md." },
  { id: "s65", phase: "phase1", epic: "Database", title: "Create stg_medications + stg_vaccines tables", status: "backlog", priority: "high", notes: "Silver staging tables. stg_medications: one row per medication period. stg_vaccines: one row per vaccine admin (tracked person only). stg_vaccines has 7 new columns: is_most_recent, administration_site, vaccination_location, batch_lot_number, vaccine_product_name, expiration_date, manufacturer. Column renamed: vaccine_name → vaccine_type_dose. See database-schema.md." },
